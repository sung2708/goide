...
function InlineActions({
  ...
  return (
    <div
      className="absolute z-10 flex items-center gap-1 rounded border border-[var(--border-subtle)] bg-[var(--mantle)] px-2 py-1 text-[11px] uppercase text-[var(--text)]"
    ...
    >
      <button
        ...
        className="rounded border border-[var(--border-subtle)] px-2 py-1 transition-colors duration-150 ease-out disabled:cursor-not-allowed disabled:opacity-60"
        disabled={!canJump}
        onClick={() => onJump?.()}
      >
        Jump
